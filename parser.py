...
@slow_watch(n=float(input('Input time(seconds) for add_items: ')))
...
@logger
@slow_watch(n=float(input('Input time (seconds) for connection_db: ')))
def connection_db():
    con = connect(
        database="Currencies",
        user="postgres",
...
@slow_watch(n=float(input('Input time (seconds) for create_tables: ')))
...
@slow_watch(n=float(input('Input time (seconds) for insert_into_tables: ')))
def insert_into_tables(con, data):
    for key, value in data.items():
        cur = con.cursor()
        query = f"""Insert into {key}
                (purchase_course, sale_course, renew_date)
